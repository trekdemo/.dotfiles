...
layouts['fullscreen-with-padding'] = function(windows)
  local space = getSpace()
  local padding = space.padding
  if padding == nil then
    padding = 20
  end
  ...
  fnutils.each(windows, function(win)
    ...
    frame.w = frame.w - (2 * padding)
    frame.h = frame.h - (2 * padding)
    win:setFrame(frame)
  end)
end
...
layouts['main-vertical-variable-with-padding'] = function(windows)
  local winCount = #windows

  if winCount == 1 then
    return layouts['fullscreen-with-padding'](windows)
  end
  ...
  for index, win in pairs(windows) do
    ...
    if index == 1 then
      frame.x = padding
      frame.y = frame.y + padding
      frame.w = (frame.w * mainVertical) - (1.5 * padding)
      frame.h = frame.h - (2 * padding)
    else
      local paddedWinHeight = (frame.h - winCount * padding) / (winCount - 1)
      frame.x = frame.x + frame.w * mainVertical + (0.5 * padding)
      frame.y = frame.y + paddedWinHeight * (index - 2) + padding * (index - 1)
      frame.w = frame.w * (1 - mainVertical) - (1.5 * padding)
      frame.h = paddedWinHeight
    end
